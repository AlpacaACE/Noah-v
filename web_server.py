...
def init_model(lm_config):
    ...
    if model_from == 1:
        ...
        model = AutoModelForCausalLM.from_pretrained('Noah-v',
    ...
    with gr.Blocks() as demo:
        ...
        gr.HTML(f"""
            <div style="text-align: center;">
                <h3>Noah-v ({(count_parameters(model) / 1e6):.2f} M) Test Demo 「单轮对话，无历史上下文」</h3>
            </div>
            """)
        # 左侧
        with gr.Row():
            ...
            with gr.Column(scale=4):
                ...
                chatbot = gr.Chatbot(label="Noah-v Chatbot", height=670)
